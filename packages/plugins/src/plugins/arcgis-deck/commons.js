...
export function render(resources, viewport) {
  ...
  if (device instanceof WebGLDevice) {
    ...
    const rawScreenFbo = device.getParametersWebGL(36006);
    ...
    const currentTexture = fbo.colorAttachments[0].texture ?? fbo.colorAttachments[0];
    if (currentTexture !== resources.texture) {
      resources.texture = currentTexture;
      model.setBindings({ deckglTexture: currentTexture });
    }
    ...
    if (rawScreenFbo) {
      gl.bindFramebuffer(36160, rawScreenFbo);
