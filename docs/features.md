...
- Multi-map grid that splits the workspace into a grid of synchronized map views, so you can compare basemaps, layers, or time steps side by side, with any **secondary** pane switchable to a CesiumJS 3D globe via its 2D/3D toggle (camera-synced with the 2D maps, and drawing the project's own basemap), and the primary map itself switchable to Cesium (see [3D globe](#3d-globe-cesium) below)
...
- **Cesium as the primary rendering engine**: View → Rendering engine switches the whole workspace between MapLibre 2D and Cesium 3D, saved with the project; the MapLibre map is unmounted rather than hidden, and the camera, basemap, layers, groups, visibility, and opacity survive the switch. Both engines implement one `MapEngine` interface, and menus, tools, and plugins are gated on the active engine's capabilities. See [Architecture](architecture.md#3d-globe-view-cesiumjs)
...
- **Globe chrome**: Reset view, fullscreen, and a 3D / 2D / Columbus scene-mode picker (flat modes in Web Mercator to match the project map), movable between corners; a native basemap and terrain gallery with Ion imagery plus keyless Esri, OpenStreetMap, and Stadia styles, with a Project basemap option; cursor coordinates, elevation, and the active projection in the shared status bar
...
- Add Data menu covering every remote and cloud-native source:
    ...
    - **[WCS raster subsets](wcs.md)**: discover WCS 1.0.0 coverages and download GeoTIFF subsets in EPSG:4326 with explicit extent and pixel dimensions, preserving numerical values for raster inspection and analysis
